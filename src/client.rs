...
impl H3ClientConn {
    ...
    pub async fn request(
        ...
        loop {
            ...
            if let Ok(Ok(len)) = recv_timeout.await {
                let recv_info = quiche::RecvInfo {
                    to: self.conn.peer_addr,
                    from: self.server_addr,
                };
            ...
            let event = {
                ...
                match h3.poll(&mut *quic) {
                    ...
                    Err(quiche::h3::Error::Done) => {
                        let mut quic2 = self.conn.quic.lock().await;
                        loop {
                            match quic2.send(&mut out) {
                                Ok((write, _)) => {
        ...
        tokio::spawn(async move {
            loop {
                ...
                };
                {
                    ...
                    let _ = quic.recv(&mut recv_buf[..len], recv_info);
                }
